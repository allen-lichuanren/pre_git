<template>
  <div class="personal_center" @click=init_modify>
    ...
    <div class="row box">
        <div class="col-lg-2 box_left offset-lg-2">
          <ul class="text-center">
            <div class="head ">
              ...
              <p class="integ">積分 <span class="num_red">{{userInfo.award_points}}</span></p>
            </div>
        ...
        <div class="box_left_m">
          <div class="head ">
              ...
              <p class="integ">{{userInfo.nickname.val}} <span class="num_red">積分{{userInfo.award_points}}</span></p>
            </div>
            ...
            <li class="col-sm-3" v-for="(tab,index) in tabLists" @click=hoho(tab) v-bind:class="{'active':tab.active==true}" v-if="index<4">
              <div class="btn-group" v-if="index==0">
                ...
                <ul class="dropdown-menu" role="menu">
                  <li><span @click.stop=hoho(tabLists[0])>修改個人資料</span></li>
                  <li><span @click.stop=hoho(tabLists[5])>修改頭像</span></li>
                </ul>
              </div>
              <div class="btn-group" v-if="index==2">
                ...
                <ul class="dropdown-menu" role="menu">
                  ...
                  <li><span @click.stop=hoho(tabLists[6])>最近瀏覽</span></li>
                </ul>
              </div>
        ...
        <div class="col-lg-6 box_right"  >
            <div v-bind:class="{'right_mation':true,'right_mation1':true,'active':tabLists[0].active==true}" >
              <div class="head"><span @click=hoho(tabLists[0]) class="span_sel">修改個人資料</span><span @click=hoho(tabLists[5])>修改頭像</span></div>
              
              <div class="form_box">
                <ul>
                  <li>昵稱：<input type="text" v-model="userInfo.nickname.val"  v-bind:disabled="userInfo.nickname.state==false" v-focus="userInfo.nickname.focus"><span class="revise" @click="revise('nickname')">{{userInfo.nickname.state==false?'修改':'取消'}}</span></li>
                  <li>性別 
                  ...
                  <!-- <li>郵箱：<input type="email" v-model="userInfo.email.val"  v-bind:disabled="userInfo.email.state==false" v-focus="userInfo.email.focus"><span class="revise" @click="revise('email')">{{userInfo.email.state==false?'修改':'取消'}}</span></li> -->
                  ...
                  <li>職位：<input type="text" v-model="userInfo.position.val"  v-bind:disabled="userInfo.position.state==false" v-focus="userInfo.position.focus"><span class="revise" @click="revise('position')">{{userInfo.position.state==false?'修改':'取消'}}</span></li>
                  <li>邀請碼：<input type="text" v-model="userInfo.invite.val"  v-bind:disabled="false"><span></span></li>
                </ul>
            ...
            <div v-bind:class="{'right_mation':true,'right_mation2':true,'active':tabLists[2].active==true}" >
              <div class="head"><span @click=hoho(tabLists[2]) class="span_sel">我的收藏</span><span @click=hoho(tabLists[6])>最近瀏覽</span></div>
              <ul>
                  ...
                  <div class="new_txt">
                      <div class="txt_mation">
                          ...
                          <div style="display: flex; justify-content: space-between;align-items: flex-end;">
                            <span>
                              <span @click=article_detial(item.aid) class="read_txt">閱讀全文</span>
              ...
              <div class="page_select pageContainer" v-if="collection.length>0">
                  <ul class="pagesInner">
                     <li v-if="collectionState.select!=1" class="page" @click="prevOrNext(-1)"><span aria-hidden="true">上壹頁</span></li>
                     ...
                     <li v-if="collectionState.select!=collectionState.pages" class="page" @click="prevOrNext(1)"><span aria-hidden="true">下壹頁</span></li>
                  </ul>
                </div>
            </div>
            <div v-bind:class="{'right_mation':true,'right_mation3':true,'active':tabLists[3].active==true}" >
              <div class="head"><span class="span_sel">我的邀請</span></div>
              <div class="invite_hint">
                  <div class="title">溫馨提示</div>
                  <ul>
                    <li>1、分享好友並註冊成功後，您可獲得2積分獎勵。</li>
                    <li>2、您邀請的好友成功再邀請其他好友，您可獲得1積分的額外獎勵。</li>
                    <li>3、二級邀請：您邀請的好友成功再邀請其他好友。</li>
                    <li>4、積分可兌換代幣哦，趕緊來分享吧~</li>
                  </ul>
              </div>
              <div class="invite_link">
                ...
                <span class="link_box">
                  ...
                  <span class="cliptext_btn" @click=clickCopy :data-clipboard-text="'http://market.centralcrypto.io/#/regist?inviteCode='+userInfo_default.invite.val" >點擊復制</span>
                </span>
              </div>
              <div class="head"><span class="span_sel">邀請記錄</span></div>
                  ...
                  <table class="table">
                    ...
                    <tbody>
                      <tr v-if="inviteRecord.length==0">
                        <td colspan=5 style="text-align: center;">暫無邀請記錄</td>
                      </tr>
            ...
            <div v-bind:class="{'right_mation':true,'right_mation4':true,'active':tabLists[1].active==true}" >
              <div class="head"><span class="span_sel">安全設置</span></div>
              <ul>
                  <li><span class="name">密碼登陸</span><span class="bind">已設置</span> <span class="suggest">建議使用8-12 個字符以上的字母與數字組合</span> <span></span><span @click.stop=toggle_modify style="cursor: pointer;" class="revise">修改</span></li>
                  <li><span class="name">綁定郵箱</span><span class="bind">已綁定</span> <span class="suggest">綁定郵箱可找回密碼，登錄平臺，重要通知等</span> <span></span><a style="opacity: 0" class="revise">修改</a></li>
                  <li><span class="name">綁定手機</span><span class="unbind">未綁定</span> <span class="suggest">綁定手機可找回密碼（適用APP），活動通知等</span> <span></span><span class="suggest">未開放</span></li>
              </ul>
              <button class="esc btn btn-primary" @click=signout>退出賬號</button>
            </div>

            <div v-bind:class="{'right_mation':true,'right_mation1':true,'active':tabLists[5].active==true}" >
              <div class="head"><span @click=hoho(tabLists[0])>修改個人資料</span><span @click=hoho(tabLists[5]) class="span_sel">修改頭像</span></div>
            ...
            <div v-bind:class="{'right_mation':true,'right_mation1':true,'active':tabLists[6].active==true}">
              <div class="head"><span @click=hoho(tabLists[2]) >我的收藏</span><span class="span_sel" @click=hoho(tabLists[6])>最近瀏覽</span></div>
    ...
    <div class="modify" v-if="modify_pass" @click.stop>
      <div class="title">修改登陸密碼</div>
      <div class="cont">
        <div><label>當前登陸郵箱</label><span>{{userInfo_default.email.val}}</span></div>
        <div><label>當前登陸密碼</label><input v-model="password[0]" type="password"/></div>
        <div><label>新登陸密碼</label><input v-model="password[1]" type="password"/><p>建議使用8-12 個字符以上的字母與數字組合</p></div>
        <div><label>確認新密碼</label><input v-model="password[2]" type="password"/></div>
      </div>
      <div class="confirm"><button @click=resetPass>確定</button></div>
    </div>
    <footerBox></footerBox>
    </div>
...
export default {
    data(){
        return {
          tabLists:[
            {'name':'個人資料','icon':require('../../img/per-icon.png'),'active':true},
            {'name':'安全設置','icon':require('../../img/set-icon.png'),'active':false},
            {'name':'我的收藏','icon':require('../../img/collect-icon.png'),'active':false},
            {'name':'我的邀請','icon':require('../../img/invite-icon.png'),'active':false},
            {'name':'幫助中心','icon':require('../../img/help-icon.png'),'active':false},
            {'name':'修改頭像','icon':require('../../img/help-icon.png'),'active':false},
            {'name':'瀏覽記錄','icon':require('../../img/help-icon.png'),'active':false},
          ],
          ...
          userInfo:{
            ...
            company:{val:'暫無',state:false,focus:false,modify:false},
            position:{val:'暫無',state:false,focus:false,modify:false},
            invite:{val:'112340'},
            create:{val:''},
            award_points:'',
          },
    ...
    methods:{
      ...
      signout(){
        api.signout().then(res=>{
          if(res.code==200){
            this.userLogout();
            com.toast('退出成功,頁面即將跳轉...');
            setTimeout(()=>{
              this.$router.replace({path:'/deepNews_td'})
            },2000)
          }else{
      ...
      getUserInfo(){
        api.getRightUserInfo().then(res=>{
          ...
          if(JSON.stringify(res)=='{}'||res.success===false){
            ...
            // company:中精,                   //單位：默認為 ''
            // email:"836023725@qq.com",             //註冊郵箱
            // gender:1,                     //性別：1 男 ；2 女 ；'' 未填寫
            // headImg:'http:192.168.31.99:7000/img/default.png',  //返回默認頭像
            // nickname:'',                    //昵稱：默認為 ''
            // position:'',                    //職位：默認為 ''
            // inviteCode
            let imgIndex=0
      ...
      getcollection(){
        api.getcollection({language:'2',pagesize:'3',page:this.collectionState.select}).then(res=>{
      ...
      clickCopy(){
          ...
          clipboard.on('error', function(e) {
              com.toast('您的瀏覽器不支持該功能，請在頁面中手動復制該鏈接');
          });
      },
      cancelCollect(aid){
        console.log(aid)
