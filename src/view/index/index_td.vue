          ...
          <div class="col-lg-3 left-side order">
            <div class="content_box" style="">
              <div class="cont_header quick_news" @click=toggle(0)>
                <img src="../../img/arrow_d.png" alt=""><span class="title">快訊</span><span class="date"><span class="day">{{quick_news.date}}</span> <span class="week">{{quick_news.week}}</span></span>
              </div>
        ...
        <div class="col-lg-9 middle-side">
          ...
          <div v-show="historyState" class="content_box" style="margin-bottom: 26px">
              <div class="cont_header" @click=toggle(4)>
                <img src="../../img/arrow_d.png" alt=""><span class="title">歷史走向</span>
              </div>
          ...
          <div class="content_box" style="">
              <div class="cont_header" @click=toggle(1)>
                <img src="../../img/arrow_d.png" alt=""><span class="title">幣種資料</span>
              </div>
                ...
                <div class="col-md-6">
                  <div class="ins_info">
                      <div class="datum_head">
                        ...
                        <div class="sub_name">{{coin_info.chinese_name_hk}}（{{coin_info.english_name}}）</div>
                    </div>
                    <div class="introduce">
                      <div class="title">簡介</div>
                      <div class="ins_cont">{{coin_info.description_hk}}</div>
                    </div>
                  </div>
                </div>
                <div class="col-md-5 offset-sm-1">
                  <div class="ins_detail">
                    <div><span>發行時間</span><span>{{coin_info.publish_time}}</span></div>
                    <div><span>發行總量</span><span>{{coin_info.market_capitalization}}</span></div>
                    <div><span>流通總量</span><span>{{coin_info.market_count}}</span></div>
                    <div><span>眾籌價格</span><span>{{coin_info.ico_price}}</span></div>
                    <div><span>白皮書</span><span><a style="display:flex;font-size: 12px;line-height: 20px;max-width: 100%;word-break: break-all;" :href="coin_info.white_paper" target="_blank">{{coin_info.white_paper}}</a></span></div>
                    <div><span>官網</span><span><a style="display:flex;font-size: 12px;line-height: 20px;max-width: 100%;word-break: break-all;" :href="coin_info.website[0]" target="_blank">{{coin_info.website[0]}}</a></span></div>
                    <div><span>區塊鏈查詢</span><span><a style="display:flex;font-size:12px;line-height: 20px;max-width: 100%;word-break: break-all;" :href="coin_info.block_explorer[0]">{{coin_info.block_explorer[0]}}</a></span></div>
                  </div>
                </div>
              </div>
          </div>
            ...
            <div class="col-lg-12" style="padding-left:0; padding-right: 0;">
                <div class="content_box" style="">
                    <div class="cont_header" @click=toggle(2)>
                      <img src="../../img/arrow_d.png" alt=""><span class="title">幣種深度新聞</span>
                    </div>
                    <div v-bind:class="{'content':true,'hide':toggle_detail[2].hide}">
                      
                      <div v-show="!articles[0].aid" class="" style="height: 180px;display: flex;align-items: center;justify-content: center;font-size: 16px">該幣種暫無新聞</div>
                      ...
                      <div v-if="articles[0].aid" v-for="item in articles" class="news" news_id='1'>
                         ...
                         <div class="news_article">
                           ...
                           <div class="news_footer">
                             <span><!-- <span class="read_all" @click=read_detail(item.aid)> 閱讀全文</span> --><span class="news_date"> {{item.resource}} &bull; {{item.published_time}}</span></span>
                    ...
                    <div v-show="articles[0].aid" v-bind:class="{'cont_footer':true,'hide':toggle_detail[2].hide}">
                        <router-link :to="{path:'/coinDeepNews_td',query:{coin:select_symbol}}">查看全部深度新聞</router-link>
                    </div>
                </div>
            </div>
            <!-- <div class="col-lg-3" style="padding-right:0;">
                <div class="content_box" style="">
                  <div class="cont_header" @click=toggle(3)>
                      <img src="../../img/arrow_d.png" alt=""><span class="title">用戶評論</span>
                  </div>
                  <div v-bind:class="{'content':true,'comment':true,'hide':toggle_detail[3].hide}">
                    <div class="comment_login" v-if="!getToken.length"><router-link to="/login">登錄 </router-link>或<router-link to="/regist"> 註冊</router-link>才能評論</div>
...
import '@/util/highchart/highcharts-zh_TD.js'
...
export default {
  ...
  "Bitfinex":{"BTC":["AGI_BTC","AID_BTC","AVT_BTC","ANT_BTC","AIO_BTC","AUC_BTC","BCH_BTC","BTG_BTC","BAT_BTC","BFT_BTC","BCI_BTC","CFI_BTC","CBT_BTC","DSH_BTC","DAT_BTC","DTH_BTC","DAI_BTC","DAD_BTC","EOS_BTC","ETH_BTC","ETC_BTC","ETP_BTC","ELF_BTC","EDO_BTC","FUN_BTC","FSN_BTC","GNT_BTC","IOT_BTC","IOS_BTC","KNC_BTC","LTC_BTC","LRC_BTC","LYM_BTC","MTN_BTC","MKR_BTC","MNA_BTC","MIT_BTC","NEO_BTC","OMG_BTC","ODE_BTC","ORS_BTC","POA_BTC","POY_BTC","QSH_BTC","QTM_BTC","RLC_BTC","REQ_BTC","RCN_BTC","REP_BTC","RDN_BTC","RRT_BTC","SNT_BTC","SAN_BTC","SPK_BTC","STJ_BTC","SNG_BTC","TRX_BTC","TNB_BTC"],"USD":["AGI_USD","AVT_USD","AID_USD","AIO_USD","ANT_USD","AUC_USD","BTC_USD","BCH_USD","BTG_USD","BAT_USD","BCI_USD","BFT_USD","CFI_USD","CBT_USD","DSH_USD","DAT_USD","DTH_USD","DAI_USD","DAD_USD","ETH_USD","EOS_USD","ETC_USD","ETP_USD","ELF_USD","EDO_USD","FUN_USD","FSN_USD","GNT_USD","IOT_USD","IOS_USD","KNC_USD","LTC_USD","LRC_USD","LYM_USD","MNA_USD","MTN_USD","MKR_USD","MIT_USD","NEO_USD","OMG_USD","ODE_USD","ORS_USD","POA_USD","POY_USD","QTM_USD","QSH_USD","RRT_USD","REP_USD","RLC_USD","RCN_USD","REQ_USD","RDN_USD","SAN_USD","SNT_USD","SPK_USD","SNG_USD","STJ_USD","TRX_USD","TNB_USD"],"ETH":["AGI_ETH","AVT_ETH","AID_ETH","ANT_ETH","AIO_ETH","AUC_ETH","BCH_ETH","BAT_ETH","BFT_ETH","CFI_ETH","CBT_ETH","DAT_ETH","DTH_ETH","DAI_ETH","DAD_ETH","EOS_ETH","ETP_ETH","ELF_ETH","EDO_ETH","FUN_ETH","FSN_ETH","GNT_ETH","IOT_ETH","IOS_ETH","KNC_ETH","LRC_ETH","LYM_ETH","MKR_ETH","MTN_ETH","MIT_ETH","MNA_ETH","NEO_ETH","OMG_ETH","ODE_ETH","ORS_ETH","POA_ETH","POY_ETH","QSH_ETH","QTM_ETH","RLC_ETH","REQ_ETH","REP_ETH","RDN_ETH","RCN_ETH","SAN_ETH","SNT_ETH","SPK_ETH","STJ_ETH","SNG_ETH","TRX_ETH","TNB_ETH"],"EUR":["BTC_EUR","ETH_EUR","EOS_EUR","IOT_EUR","NEO_EUR"],"JPY":["BTC_JPY","ETH_JPY","EOS_JPY","IOT_JPY","NEO_JPY"],"GBP":["BTC_GBP","ETH_GBP","EOS_GBP","IOT_GBP","NEO_GBP"]}
            ...
            coin_info:{
              "description_zh":"Aeternity號稱新壹代區塊鏈網絡,由德裔計算機科學家“以太坊教父”Yanislav Malahov與前Augur核心開發者Zack Hess、前Synereo核心開發者Jack Pettersson於2016年發起，聚集諸多顛覆性創新於壹身，旨在重樹當今智能合約的底層協議。Aeternity的技術亮點是圖靈完備狀態通道(State Channels)和去中心化預言機（Oracles）。",
    ...
    methods:{
        ...
        getData(){
            ...
            //獲取幣種信息
            var data={coin:this.select_symbol.split('_')[0],language:2}
            ...
            var week=["星期壹","星期二","星期三","星期四","星期五","星期六","星期日"]
            api.getQuickNews({page:1,pagesize:3,language:2}).then(res=>{
              ...
              if(!res.news||!res.news.length){
                this.quick_news.data=[{created_at:null,content:'今日暫無快訊'}];
                return
              }
        ...
        getNews(){
          ...
          api.getArticles({coin:code,pagesize:3,page:1,language:2}).then(res=>{
