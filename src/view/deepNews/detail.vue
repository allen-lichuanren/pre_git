<template>
  <div class="news_detail">
      ...
      <div class="row top" style="z-index: 0">
        ...
        <div class="col-lg-3">
          <div class="aside">
            <!-- <div class="aside_title">相关推荐</div>
            <div class="aside_item">
              <img src="../../img/news.png" alt="">
              <div class="suggest_con">
                <div>油价飙升会导致 股市崩盘吗？</div>
            ...
            <!-- <div class="recent_view">
              <img src="../../img/dot.png" alt="">
              <div>日本经济学家解释为什么比特币价格不会迅速上涨</div>
            </div>
            <div class="recent_view">
              <img src="../../img/dot.png" alt="">
              <div>动态 | Syscoin遭遇黑客攻击</div>
            </div>
            <div class="recent_view">
              <img src="../../img/dot.png" alt="">
              <div>币安临时维护 市场传言异常交易或与Syscoin漏洞有关</div>
            </div>
            <div class="recent_view">
              <img src="../../img/dot.png" alt="">
              <div>区块链产业 进入高速发展阶段</div>
            </div> -->
          </div>
        </div>
      </div>
          ...
          <div class="col-lg-9">
             <div class="assess">
               <div class="assess_head">
                 <span class="num">共{{judge_total.number}}条评论</span>
                 <span class="sort">按热度排序</span>
               </div>
               <div class="assess_list">
                <div v-if="judgement.length==0" style="min-height: 200px;display: flex;align-items: center;justify-content: center;">
                  暂无评论
                </div>
                 <div class="assess_item" v-for="item in judgement">
                    ...
                    <div class="item_footer">
                      ...
                      <span class="zan" @click=zan(item)><img src="../../img/like_r.png" alt=""> <span>{{item.favorite}}</span></span>
                    </div>
                 </div>


                <div class="page_select pageContainer" v-if="judgement.length>0">
                  <ul class="pagesInner">
                     <li v-if="judge_total.select!=1" class="page" @click="prevOrNext(-1)"><span aria-hidden="true">上一页</span></li>
                     ...
                     <li v-if="judge_total.select<judge_total.pages" class="page" @click="prevOrNext(1)"><span aria-hidden="true">下一页</span></li>
                  </ul>
                </div>
               </div>
               ...
               <div class="do_assess">
                 <input type="text" name="" placeholder="写下您的评论" v-model="judge_text">
                 <span class="access_btn" @click=judge>发表</span>
               </div>
              </div>
...
export default {
    data(){
        return {
          ...
          fixed:true,
          judge_text:'',
          judge_total:{pages:1,number:0,select:1},
    ...
    methods:{
        ...
        getData(){
           api.getArticles({aid:this.aid,lanuage:1}).then(res=>{
        ...
        judge(){
          ...
          if(this.judge_text.length>200){
            com.toast("评论不能超过200字")
            return
          }
          api.judge({aid:this.aid,content:this.judge_text}).then(res=>{
            if(res.code==200){
              com.toast('评论成功');
              //刷新评论数据
