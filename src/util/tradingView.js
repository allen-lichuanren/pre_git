"use strict"; (function() {
    ...
    var TradingView = {
        ...
        QuotesProvider: function(options) {
            ...
            TradingView.bindEvent(iframe, "load",
            function() {
            ...
            function resolveSymbol(symbolObj) {
                ...
                if (success) {
                    postMessage.on("success" + symbol,
                    function(data) {
                        success(data)
                    })
                }
                if (error) {
                    postMessage.on("error" + symbol,
                    function(data) {
                        error(data)
                    })
                }
    ...
    TradingView.MiniWidget.prototype = {
        ...
        widgetCode: function() {
            ...
            if (this.options.tabs) {
                for (var i = 0,
                tl = this.options.tabs.length; i < tl; i++) {
    ...
    TradingView.widget.prototype = {
        create: function() {
            ...
            TradingView.bindEvent(this.iframe, "load",
            function() {
                self.postMessage.get("widgetReady", {},
                function() {
                    var i;
                    self._ready = true;
            ...
            self.postMessage.on("logoCreated",
            function(data) {
        ...
        saveChart: function(callback) {
            this.postMessage.get("chartID", {},
            callback)
        },
        getSymbolInfo: function(callback) {
            this.postMessage.get("symbolInfo", {},
            callback)
        },
        remove: function() {
    ...
    TradingView.showSignIn = function(options, callback) {
        TradingView.dependenciesManager.depends(["/static/bundles/spinner.js"], "authWidget",
        function() {
            var shadowBox = document.createElement("div");
            var spinnerContainer = document.createElement("div");
            shadowBox.appendChild(spinnerContainer); (new window.Spinner).setStyle({
                color: "#00A2E2",
                opacity: "0"
